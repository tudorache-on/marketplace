...
public interface LikesDislikesMapper {
    ...
    @Select("SELECT COUNT(*) FROM likes_dislikes WHERE user_id = #{user_id} AND product_id = #{product_id}")
    int existsByUserIdAndProductId(@Param("user_id") long userId, @Param("product_id") long productId);
